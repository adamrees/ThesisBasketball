...
yVals <- function(xLocs, yLocs, data){
  ...
  for(j in 1:length(yLocs)){
    if(!is.na(data$X_LOC[counter]) & data$X_LOC[counter] == xLocs[i] & data$Y_LOC[counter] == yLocs[j]){
  ...
  finalVector <- data.frame(xLocs, yLocs, tempY, tempSig)
...
grid.b <- seq(-31.6874,295.7334,10.5656)
