...
sched
played <- which(sched$away_team=='PHX' | sched$home_team=='PHX')[1:4]


total <- c()

for (j in 1:length(played)){
  tempData <- nba.data(played[[j]])
...
make.url <- function(game) {
  ...
  awayName <- gsub(" ", "-",   sched[game,]$away_team_full_name)
  awayName <- ifelse(awayName == "LA-Clippers", "Los-Angeles-Clippers", awayName)
  homeName <- gsub(" ", "-", sched[game,]$home_team_full_name)
  homeName <- ifelse(homeName == "LA-Clippers", "Los-Angeles-Clippers", homeName)

  game.url <- str_to_lower(paste(sep="", 'https://sports.yahoo.com/nba/', awayName,'-', homeName,'-','20',year,mo,day, TeamInfo$TeamNum[which(TeamInfo$Team==toString(sched$home_team[game]))],'/?section=plays'))
  game.url
}
