...
for(i in 1:numShots){
  ...
  while(!is.na(subs[[1]][subsCounter + 1]) &&
    ...
    enterPlayer <- as.numeric(substr(data, subs[[1]][subsCounter] + 51, subs[[1]][subsCounter] + 54))
    leavePlayer <- as.numeric(substr(data, subs[[1]][3] + 73, subs[[1]][3] + 76))
    
    if(shotLocations$H1[i] == leavePlayer){
      shotLocations$H1[i] <- enterPlayer
    }
    if(shotLocations$H2[i] == leavePlayer){
      shotLocations$H2[i] <- enterPlayer
    }
    if(shotLocations$H3[i] == leavePlayer){
      shotLocations$H3[i] <- enterPlayer
    }
    if(shotLocations$H4[i] == leavePlayer){
      shotLocations$H4[i] <- enterPlayer
    }
    if(shotLocations$H5[i] == leavePlayer){
      shotLocations$H5[i] <- enterPlayer
    }
    if(shotLocations$A1[i] == leavePlayer){
      shotLocations$A1[i] <- enterPlayer
    }
    if(shotLocations$A2[i] == leavePlayer){
      shotLocations$A2[i] <- enterPlayer
    }
    if(shotLocations$A3[i] == leavePlayer){
      shotLocations$A3[i] == enterPlayer
    }
    if(shotLocations$A4[i] == leavePlayer){
