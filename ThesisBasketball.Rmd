...
```{r}
GameSchedule <- read.csv("~/Desktop/Thesis/ThesisBasketball/GameSchedule", sep="")
head(GameSchedule)
```

```{r}
for(i in 1:nrow(GameSchedule)){
  GameSchedule$Month[i] <- substr(toString(GameSchedule$Date[i]), 0, 2)
  GameSchedule$Day[i] <- substr(toString(GameSchedule$Date[i]), 4, 5)
...
TeamInfo %>% filter(Team == "MIL") %>% select(FullLocation)
```
